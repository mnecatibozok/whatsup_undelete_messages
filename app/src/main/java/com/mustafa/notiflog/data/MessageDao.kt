...
interface MessageDao {
    ...
    // ---- Ana ekran: doğrudan silinen mesajların düz listesi ----

    @Query("SELECT * FROM messages WHERE isDeleted = 1 ORDER BY timestamp DESC")
    fun getDeletedOnly(): LiveData<List<MessageEntity>>

    @Query("""
        SELECT * FROM messages 
        WHERE isDeleted = 1
          AND (conversationTitle LIKE '%' || :query || '%' 
               OR sender LIKE '%' || :query || '%'
               OR text LIKE '%' || :query || '%')
        ORDER BY timestamp DESC
    """)
    fun searchDeleted(query: String): LiveData<List<MessageEntity>>
